...
public class BulkUploadController {
    ...
    public ResponseEntity<?> startUpload(@RequestParam("file") MultipartFile file) {
        ...
        log.info("BulkUploadController.startUpload invoked user={} filename={} size={} bytes", currentUser,
        ...
        try {
            if (file == null || file.isEmpty()) {
                ...
                return ResponseEntity.badRequest()
                        .body(new ApiFailureResponse<>(Collections.emptyList(), "No file provided", 400));
            }

            if (file.getSize() > MAX_FILE_SIZE) {
                log.warn("Validation failed: File too large user={} filename={} size={} bytes", currentUser,
                        originalFilename, file.getSize());
                return ResponseEntity.status(HttpStatus.PAYLOAD_TOO_LARGE)
                        .body(new ApiFailureResponse<>(Collections.emptyList(),
                                "File too large (max " + (MAX_FILE_SIZE / (1024 * 1024)) + " MB)", 413));
            }

            if (!com.meritcap.helper.ExcelHelper.checkExcelFormat(file)) {
                log.warn("Validation failed: Incorrect Excel format user={} filename={}", currentUser,
                        originalFilename);
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(new ApiFailureResponse<>(Collections.emptyList(),
                                "Incorrect excel format; expected .xlsx/.xls", 400));
            }

            BulkUploadResponseDto resp = bulkUploadService.startBulkUpload(file, currentUser);
            log.info("Bulk upload job created successfully user={} filename={} jobId={}", currentUser, originalFilename,
                    resp.getJobId());

            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(new ApiSuccessResponse<>(resp, "Bulk upload started", 201));

        } catch (IllegalArgumentException ex) {
            log.warn("Validation exception user={} filename={} error={}", currentUser, originalFilename,
                    ex.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ApiFailureResponse<>(Collections.emptyList(), ex.getMessage(), 400));
        } catch (Exception e) {
            log.error("Unexpected error starting bulk upload user={} filename={} size={} bytes", currentUser,
                    originalFilename, fileSize, e);
    ...
    public ResponseEntity<?> listJobs() {
        ...
        try {
            List<BulkUploadStatusDto> jobs = bulkUploadService.listRecentJobs();
            return ResponseEntity.ok(new ApiSuccessResponse<>(jobs, "Jobs fetched", 200));
        } catch (Exception e) {
            log.error("Error listing jobs user={}", currentUser, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiFailureResponse<>(Collections.emptyList(), "Failed to list jobs", 500));
        }
    }
    ...
    public ResponseEntity<?> getStatus(@PathVariable("jobId") Long jobId) {
        ...
        try {
            ...
            log.debug("Fetched job status successfully user={} jobId={} status={}", currentUser, jobId,
                    status.getStatus());
