...
public class College {
    ...
    @NotBlank
    @Size(min = 3, max = 64)
    // Optionally enforce: @Pattern(regexp = "^[A-Z0-9_-]+$", message = "Only A-Z, 0-9, _, - allowed")
    ...
    private void normalize() {
        if (slug != null) slug = slug.trim().toLowerCase().replaceAll("\\s+", "-");
        if (campusCode != null) campusCode = campusCode.trim().toUpperCase();
        if (name != null) name = name.trim().replaceAll("\\s+", " ");
        if (campusName != null) campusName = campusName.trim().replaceAll("\\s+", " ");
        if (country != null) country = country.trim().replaceAll("\\s+", " ");
    }
}
