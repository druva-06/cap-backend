...
public final class NormalizationUtil {
    ...
    public static String genSlug(String name, String campus) {
        ...
        if (base.isBlank()) base = "college-" + UUID.randomUUID().toString().substring(0,8);
    ...
    public static Integer parseDurationMonths(String raw) {
        ...
        try {
            if (s.matches(".*\\d+.*")) {
                ...
                if (s.contains("year") || s.contains("yr")) return n * 12;
                return n;
            }
    ...
    public static Integer defaultIntakeYearIfMissing(Integer value) {
        if (value == null) return 0; // as per your instruction
        return value;
    }
    ...
    public static String nullifyIfEmpty(String s) {
        ...
        String t = s.trim();
        if (t.isEmpty() || t.equalsIgnoreCase("NA")) return null;
        return t;
    }
